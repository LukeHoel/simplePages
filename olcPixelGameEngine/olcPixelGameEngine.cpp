...
Sprite::Sprite(std::string sImageFile) { LoadFromFile(sImageFile); }
...
olc::rcode Sprite::LoadFromPGESprFile(std::string sImageFile,
...
olc::rcode Sprite::LoadFromFile(std::string sImageFile,
  ...
  for (int x = 0; x < width; x++)
    for (int y = 0; y < height; y++) {
      ...
      bmp->GetPixel(x, y, &c);
      SetPixel(x, y,
               Pixel(c.GetRed(), c.GetGreen(), c.GetBlue(), c.GetAlpha()));
    }
  delete bmp;
  return olc::OK;
#else
...
olc::rcode PixelGameEngine::Construct(uint32_t screen_w, uint32_t screen_h,
  ...
  if (nPixelWidth == 0 || nPixelHeight == 0 || nScreenWidth == 0 ||
      nScreenHeight == 0)
    return olc::FAIL;
...
bool PixelGameEngine::IsFocused() { return bHasInputFocus; }

HWButton PixelGameEngine::GetKey(Key k) { return pKeyboardState[k]; }

HWButton PixelGameEngine::GetMouse(uint32_t b) { return pMouseState[b]; }

int32_t PixelGameEngine::GetMouseX() { return nMousePosX; }

int32_t PixelGameEngine::GetMouseY() { return nMousePosY; }

int32_t PixelGameEngine::GetMouseWheel() { return nMouseWheelDelta; }
...
void PixelGameEngine::olc_UpdateMouse(int32_t x, int32_t y) {
  ...
  nMousePosXcache = (int32_t)(
      ((float)x / (float)(nWindowWidth - (nViewX * 2)) * (float)nScreenWidth));
  nMousePosYcache = (int32_t)((
      (float)y / (float)(nWindowHeight - (nViewY * 2)) * (float)nScreenHeight));
...
void PixelGameEngine::EngineThread() {
  ...
  glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, nScreenWidth, nScreenHeight, 0,
               GL_RGBA, GL_UNSIGNED_BYTE, pDefaultDrawTarget->GetData());
  ...
  while (bAtomActive) {
    ...
    while (bAtomActive) {
      ...
      // TODO: This is a bit slow (especially in debug, but 100x faster in
      // release mode???) Copy pixel array into texture
...
HWND PixelGameEngine::olc_WindowCreate() {
  ...
  olc_hWnd = CreateWindowEx(dwExStyle, L"OLC_PIXEL_GAME_ENGINE", L"", dwStyle,
                            nCosmeticOffset, nCosmeticOffset, width, height,
...
LRESULT CALLBACK PixelGameEngine::olc_WindowEvent(HWND hWnd, UINT uMsg,
                                                  WPARAM wParam,
                                                  LPARAM lParam) {
...
Display *PixelGameEngine::olc_WindowCreate() {
  ...
  olc_Window = XCreateWindow(
      olc_Display, olc_WindowRoot, 30, 30, nScreenWidth * nPixelWidth,
      nScreenHeight * nPixelHeight, 0, olc_VisualInfo->depth, InputOutput,
      olc_VisualInfo->visual, CWColormap | CWEventMask, &olc_SetWindowAttribs);
